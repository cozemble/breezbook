...
test("booking that ends before resource availability ends", () => {
    const resourceAvailability = [resourceDayAvailability(van1, [availabilityBlock(dayAndTimePeriod(isoDate("2021-05-23"), timePeriod(nineAm, sixPm)))])];
    const bookings = [booking(customerId("1"), carWash.id, isoDate("2021-05-23"), timeslotSpec(nineAm, tenAm, "09:00 - 10:00"), [])];
    expect(applyBookingsToResourceAvailability(resourceAvailability, bookings, services)).toEqual([resourceDayAvailability(van1, [availabilityBlock(dayAndTimePeriod(isoDate("2021-05-23"), timePeriod(tenAm, sixPm)))])]);
})
...
test("booking that starts after resource availability starts", () => {
    ...
    expect(applyBookingsToResourceAvailability(resourceAvailability, bookings, services)).toEqual([resourceDayAvailability(van1, [availabilityBlock(dayAndTimePeriod(isoDate("2021-05-23"), timePeriod(nineAm, tenAm)))])]);
})
...
test("booking that starts and ends in the middle of a resource availability", () => {
    ...
    expect(applyBookingsToResourceAvailability(resourceAvailability, bookings, services)).toEqual([resourceDayAvailability(van1, [availabilityBlock(dayAndTimePeriod(isoDate("2021-05-23"), timePeriod(nineAm, tenAm))), availabilityBlock(dayAndTimePeriod(isoDate("2021-05-23"), timePeriod(elevenAm, sixPm)))])]);
})