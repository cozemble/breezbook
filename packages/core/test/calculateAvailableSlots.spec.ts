...
const now = dayAndTime(date, nineAm);
...
describe('given a chatbot service that requires no resources, and is available monday to friday 9am to 5pm', () => {
	...
	test('if the service takes one hour, we can do 8 services a day', () => {
		const available = expectSlots(availability.calculateAvailableSlots(config, [], serviceRequest(theService, date), now));
	...
	test('if the service takes one hour, and the minimum notice period is two hours, we can do 6 services a day', () => {
		const mutatedService = {
			...theService,
			scheduleConfig: { ...theService.scheduleConfig, minimumNoticePeriod: minimumNoticePeriod(hours(2)) }
		};
		const available = expectSlots(availability.calculateAvailableSlots(config, [], serviceRequest(mutatedService, date), now));
		const times = available.map(a => availableSlotTimeFns.getStartTime(a.startTime));
	...
	test('having lots of bookings makes no difference, as no resources are required', () => {
		...
		const available = expectSlots(availability.calculateAvailableSlots(config, bookings, serviceRequest(theService, date), now));
		const times = available.map(a => availableSlotTimeFns.getStartTime(a.startTime));
	...
	test('if an hour is blocked out, we can do 7 services a day', () => {
		...
		const available = expectSlots(availability.calculateAvailableSlots(mutatedConfig, [], serviceRequest(theService, date), now));
		const times = available.map(a => availableSlotTimeFns.getStartTime(a.startTime));
	...
	test('if the service takes two hours, we can do 7 services a day', () => {
		...
		const available = expectSlots(availability.calculateAvailableSlots(config, [], serviceRequest(longerService, date), now));
		const times = available.map(a => availableSlotTimeFns.getStartTime(a.startTime));
	...
	test('good error message if there is no availability for the given day', () => {
		...
		const outcome = availability.calculateAvailableSlots(mutatedConfig, [], serviceRequest(theService, date), now) as ErrorResponse;
...
describe('given a carwash service that requires one of several interchangeable washers for each service, and is available monday to friday 9am to 5pm', () => {
	...
	test('there is no availability where there are no resources', () => {
		const slots = expectSlots(availability.calculateAvailableSlots(config, [], serviceRequest(theService, date), now));
	...
	test('we lose availability when a booking takes all the resources', () => {
		...
		const available = expectSlots(availability.calculateAvailableSlots(mutatedConfig, [theBooking], serviceRequest(theService, date), now));
		const times = available.map(a => availableSlotTimeFns.getStartTime(a.startTime));
		expect(times).toEqual([time24('09:00'), time24('11:00'), time24('12:00'), time24('13:00'), time24('14:00'), time24('15:00'), time24('16:00')]);
	});
...
describe('given a mobile carwash service that requires one of several interchangeable vans for each service, and books against time slots, and is available monday to friday 9am to 5pm', () => {
	...
	test('there is no availability where there are no resources', () => {
		const outcome = expectSlots(availability.calculateAvailableSlots(config, [], serviceRequest(theService, date), now));
	...
	test('we have availability when we have one resource', () => {
		...
		const available = expectSlots(availability.calculateAvailableSlots(mutatedConfig, [], serviceRequest(theService, date), now));
		const startTimes = available.map(a => availableSlotTimeFns.getStartTime(a.startTime));
		expect(startTimes).toEqual([time24('09:00'), time24('13:00')]);
		const endTimes = available.map(a => availableSlotTimeFns.getEndTime(a.startTime));
...
describe('given a gym that offers personal training with specific trainers, and is available monday to friday 9am to 5pm', () => {
	...
	test('we can have a personal trainer booked back to back', () => {
		...
		const mutatedConfig: AvailabilityConfiguration = { ...config, resourceAvailability: bothPtsAvailable };
		const available = expectSlots(availability.calculateAvailableSlots(mutatedConfig, [firstBooking, secondBooking], serviceRequest(theService, date), now));
		const times = available.map(a => availableSlotTimeFns.getStartTime(a.startTime));
		expect(times).toEqual([time24('11:00'), time24('12:00'), time24('13:00'), time24('14:00'), time24('15:00'), time24('16:00')]);
...
describe('given a medical centre and an appointment type that requires two doctors', () => {
	...
	test('one doctor can be specified explictly', () => {
		...
		const available = expectSlots(availability.calculateAvailableSlots(mutatedConfig, [], serviceRequest(mutatedService, date), now));
		const times = available.map((a) => availableSlotTimeFns.getStartTime(a.startTime));
...
describe('given a gym that offers personal training requiring a specific trainer and a specific gym room, and is available monday to friday 9am to 5pm', () => {
	...
	test('there is no availability when either the personal trainer or the gym room is unavailable', () => {
		...
		const trainerOnlyOutcome = expectSlots(availability.calculateAvailableSlots(trainerOnlyConfig, [], serviceRequest(theService, date), now));
		...
		const roomOnlyOutcome = expectSlots(availability.calculateAvailableSlots(roomOnlyConfig, [], serviceRequest(theService, date), now));
	...
	test('we lose availability when a booking takes either the personal trainer or the gym room', () => {
		...
		const trainerBookingAvailable = expectSlots(availability.calculateAvailableSlots(mutatedConfig, [trainerBooking, roomBooking], serviceRequest(theService, date), now));
		const trainerBookingTimes = trainerBookingAvailable.map((a) => availableSlotTimeFns.getStartTime(a.startTime));
...
describe('given a dog walking service that can take up to 6 dogs at 09.00, and is open monday to friday', () => {
	...
	test('capacity consumption is tracked across the aggregate of bookings', () => {
		...
		const available = expectSlots(availability.calculateAvailableSlots(mutatedConfig, [fourDogs, oneDog], serviceRequest(theService, date), now));

		expect(available.map((a) => availableSlotTimeFns.getStartTime(a.startTime))).toEqual([time24('09:00')]);

		const outcome2 = expectSlots(availability.calculateAvailableSlots(mutatedConfig, [fourDogs, oneDog, oneMoreDog], serviceRequest(theService, date), now));
		expect(outcome2.map((a) => availableSlotTimeFns.getStartTime(a.startTime))).toEqual([]);
	});
});
...
describe('given a hair salon that offers configurable services, and is available monday to friday 9am to 5pm', () => {
	...
	test('there is availability when we have the resources', () => {
		...
		const available = expectSlots(availability.calculateAvailableSlots(mutatedConfig, [], serviceRequest(hairStylingService, date), now));
		const times = available.map((a) => availableSlotTimeFns.getStartTime(a.startTime));
		expect(times).toEqual([time24('09:00'), time24('10:00'), time24('11:00'), time24('12:00'), time24('13:00'), time24('14:00'), time24('15:00'), time24('16:00')]);
	});
	...
	test('there is no availability when we have no resources', () => {
		const outcome = expectSlots(availability.calculateAvailableSlots(config, [], serviceRequest(hairStylingService, date), now));
		expect(outcome).toHaveLength(0);
	});
	...
	test('adding a service option that lengthens the service, reduces the number of available slots', () => {
		...
		const available = expectSlots(availability.calculateAvailableSlots(mutatedConfig, [], serviceRequest(hairStylingService, date, [], [serviceOptionAndQuantity(curling, 1)]), now));
		const times = available.map((a) => availableSlotTimeFns.getStartTime(a.startTime));
		expect(times).toEqual([time24('09:00'), time24('10:00'), time24('11:00'), time24('12:00'), time24('13:00'), time24('14:00'), time24('15:00')]);
	});
	...
	test('adding a service option that requires a resource, drops availabilty when the resource is not available', () => {
		...
		const available = expectSlots(availability.calculateAvailableSlots(mutatedConfig, [], serviceRequest(hairStylingService, date, [], [serviceOptionAndQuantity(colouring, 1)]), now));
		const times = available.map((a) => availableSlotTimeFns.getStartTime(a.startTime));
	...
	test('service option resource requirements are reduced to the min required', () => {
		...
		const available = expectSlots(availability.calculateAvailableSlots(mutatedConfig, [], serviceRequest(hairStylingService, date, [], [serviceOptionAndQuantity(curling, 1), serviceOptionAndQuantity(cutting, 1)]), now));
		const startTimes = available.map((a) => availableSlotTimeFns.getStartTime(a.startTime));
...
describe('given a yoga studio with two instructors and two rooms', () => {
	...
	test('we have availability if we have only one booking', () => {
		...
		const outcome = availability.calculateAvailableSlots(mutatedConfig, [theBooking], serviceRequest(theService, date), now);
...
describe('given a chatbot service that has capacity', () => {
	...
	test('consumed capacity is calculated correctly when there are bookings', () => {
		...
		const available = expectSlots(availability.calculateAvailableSlots(config, [booking1, booking2], serviceRequest(theService, date), now));

		expect(available.find(a => availableSlotTimeFns.getStartTime(a.startTime).value === '09:00')?.consumedCapacity.value).toBe(2);
		expect(available.find(a => availableSlotTimeFns.getStartTime(a.startTime).value === '10:00')?.consumedCapacity.value).toBe(3);
		available.filter(a => availableSlotTimeFns.getStartTime(a.startTime).value !== '09:00' && availableSlotTimeFns.getStartTime(a.startTime).value !== '10:00').forEach(slot => {
			expect(slot.consumedCapacity.value).toBe(0);
		});
	});
});
...
function expectNoAvailability(config: AvailabilityConfiguration, serviceRequest: ServiceRequest, bookings: Booking[] = []) {
	const outcome = availability.calculateAvailableSlots(config, bookings, serviceRequest, now);
	if (outcome._type === 'error.response') {
		throw errorResponseFns.toError(outcome);
	}
