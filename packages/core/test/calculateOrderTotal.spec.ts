...
test('total of order with one line and one add on should be the price of the service plus the price of the add on', () => {
	const theOrder = order(mike, [
		orderLine(carwash.smallCarWash.id,carwash.locations.london, carwash.smallCarWash.price, [addOnOrder(carwash.wax.id, 1)], isoDate(), carwash.nineToOne, [])
	]);
	const total = calculateOrderTotal(theOrder, carwash.addOns, coupons);
...
test('total of order with two lines should be the sum of the prices of the services', () => {
	const theOrder = order(mike, [
		orderLine(carwash.smallCarWash.id, carwash.locations.london,carwash.smallCarWash.price, [], isoDate(), carwash.nineToOne, []),
		orderLine(carwash.mediumCarWash.id, carwash.locations.london,carwash.mediumCarWash.price, [], isoDate(), carwash.oneToFour, [])
	]);
	const total = calculateOrderTotal(theOrder, carwash.addOns, coupons);
...
test('total of order with two lines and one add on should be the sum of the prices of the services plus the price of the add on', () => {
	const theOrder = order(mike, [
		orderLine(carwash.smallCarWash.id,carwash.locations.london, carwash.smallCarWash.price, [], isoDate(), carwash.nineToOne, []),
		orderLine(carwash.mediumCarWash.id,carwash.locations.london, carwash.mediumCarWash.price, [addOnOrder(carwash.wax.id, 1)], isoDate(), carwash.oneToFour, [])
	]);
	const total = calculateOrderTotal(theOrder, carwash.addOns, coupons);
...
test('coupon can be applied to order', () => {
	const theOrder = orderFns.addCoupon(
		order(mike, [orderLine(carwash.smallCarWash.id,carwash.locations.london, carwash.smallCarWash.price, [], isoDate(), carwash.nineToOne, [])]),
...
test('supplied service price is used instead of the price from the service to support dynamic pricing', () => {
	const theOrder = order(mike, [
		orderLine(carwash.smallCarWash.id,carwash.locations.london, price(100, currency('GBP')), [], isoDate(), carwash.nineToOne, []),
		orderLine(carwash.mediumCarWash.id,carwash.locations.london, price(200, currency('GBP')), [], isoDate(), carwash.oneToFour, [])
	]);
	const total = calculateOrderTotal(theOrder, carwash.addOns, coupons);
