import {
    addOnId,
    carwash,
    type Customer, customerId, formId, id,
    isoDate,
    mandatory,
    orderLine,
    type OrderLine,
...
export type OrderLineInput = {
    [K in OrderLineKeys]?:
    OrderLine[K] extends unknown[] ? string[] :
        string;
...
export function toOrderLine(input: OrderLineInput): OrderLine {
    ...
    const slot = mandatory(carwash.timeslots.find(slot => slot._type === "timeslot.spec" && slot.description === mandatory(input.slot, 'No slot')), 'No slot')
...
export type CustomerInput = {
    [K in CustomerKeys]?:
    Customer[K] extends unknown[] ? string[] :
        string;
