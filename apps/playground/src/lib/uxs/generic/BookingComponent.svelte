<script lang="ts">
	...
	function getServiceLocation(service: Service): ServiceLocation {
		return mandatory(tenant?.serviceLocations.find(sl => sl.serviceId === service.id), `Service location not found`);
	}
</script>
...
<div class="flex justify-center items-center min-h-screen bg-base-200">
	{#if isLoading}
		<div class="text-center">
			...
			<p class="mt-4 text-lg font-semibold">Loading {tenant?.name ?? ''}...</p>
		</div>
	{:else if tenant}
