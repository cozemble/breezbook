...
import { capacity, couponCode } from '@breezbook/packages-types';
import { isoDateFns, timePeriodFns, timezones } from '@breezbook/packages-date-time';
...
test('not possible to make an order in the past', () => {
	const dayInThePast = isoDateFns.addDays(isoDateFns.today(timezones.utc), -1);
