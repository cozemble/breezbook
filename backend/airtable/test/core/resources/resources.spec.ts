...
import {environmentId, resourceType, tenantEnvironment, tenantId} from "@breezbook/packages-core";
...
const tenantEnv = tenantEnvironment(env, tenant);
...
describe("given a multi-location gym", () => {
    ...
    test("error if resource type is unknown", async () => {
        const outcome = await resources.listByType(prisma, tenantEnv, resourceType("x")) as ErrorResponse;
    ...
    test("return resource details when found", async () => {
        const outcome = await resources.listByType(prisma, tenantEnv, resourceType("personal.trainer")) as ResourceSummary[];
