...
import { IsoDate, isoDateFns, timePeriodFns, timezones } from '@breezbook/packages-date-time';
import {
	capacity,
	environmentId,
	mandatory,
...
async function completeCancellationGrant() {
	const bookingId = await createBooking(isoDateFns.addDays(isoDateFns.today(timezones.utc), 3));
...
async function dbCarwashTenantReferenceData(): Promise<EverythingToCreateOrderReferenceData> {
	...
	const availabilityData = await byLocation.getEverythingForAvailability(prisma, tenantEnvironmentLocation(dbCarwashTenant.environmentId, dbCarwashTenant.tenantId, dbCarwashTenant.locations.london), today, today);
