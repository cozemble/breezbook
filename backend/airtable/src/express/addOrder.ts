    ...
    await withTwoRequestParams(req, res, tenantIdParam(), orderBody(), async (tenantId, order) => {
        ...
        await withAdminPgClient(async (client) => {
            ...
            for (const line of order.lines) {
                ...
                await client.query(`insert into order_lines (tenant_id, order_id, service_id, time_slot_id, start_time_24hr, end_time_24hr, add_on_ids, date)
                                    values ($1, $2, $3, $4, $5, $6, $7, $8)`, [tenantId.value, orderId, line.serviceId.value, time_slot_id, servicePeriod.from.value, servicePeriod.to.value, addOnsJsonbArray, line.date.value]);
                const bookingObject = booking(customerId(customerIdValue), line.serviceId, line.date, line.slot);
                await client.query(`insert into bookings (tenant_id, customer_id, service_id, order_id, date, start_time_24hr, end_time_24hr, definition, id) values ($1, $2, $3, $4, $5, $6, $7, $8, $9)`,
