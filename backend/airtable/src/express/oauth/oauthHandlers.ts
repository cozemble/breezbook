...
import { paramExtractor, ParamExtractor, path, RequestValueExtractor, tenantEnvironmentParam, withTwoRequestParams } from '../../infra/functionalExpress.js';
...
export async function onGetAccessToken(req: express.Request, res: express.Response): Promise<void> {
	await withTwoRequestParams(req, res, tenantEnvironmentParam(), systemIdParam(), async (tenantEnvironment, systemParam) => {
		...
		const { access_token, expires_in, fetchTimestamp, refresh_token, refresh_expires_in } = await refreshToken(maybeRefreshToken.token);
