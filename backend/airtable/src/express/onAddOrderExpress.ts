...
import { Duration, IsoDate, isoDateFns, TwentyFourHourClockTime } from '@breezbook/packages-date-time';
...
async function handleAddOrder(deps: EndpointDependencies, tenantEnvironment: TenantEnvironment, orderRequest: PricedCreateOrderRequest): Promise<EndpointOutcome[]> {
	...
	const locations = pricedBasketFns.getLocations(orderRequest.basket);
	if (!values.allSame(locations)) {
		return [httpResponseOutcome(responseOf(400, JSON.stringify({ error: 'All services must be at the same location' }), ['Content-Type', 'application/json']))];
	}
	const location = mandatory(locations[0], 'Location missing');
	const tel = tenantEnvironmentLocation(tenantEnvironment.environmentId, tenantEnvironment.tenantId, location);
	const everythingForAvailability = await byLocation.getEverythingForAvailability(prisma, tel, fromDate, toDate);
