...
export interface PricedSlot {
    _type: 'priced.slot'
    slot: ResourcedTimeSlot
    price: Price
    adjustments: PriceAdjustment[]
}

export function pricedSlot(slot: ResourcedTimeSlot, price: Price, adjustments: PriceAdjustment[]): PricedSlot {
    return {
        _type: 'priced.slot',
        slot,
        price,
...
function applyPriceAdjustment(slot: PricedSlot, pricingRule: TimeBasedPriceAdjustment): PricedSlot {
    if (pricingRule.adjustment._type === 'amount.based.price.adjustment') {
        return pricedSlot(slot.slot, price(slot.price.amount.value + pricingRule.adjustment.amount.amount.value, slot.price.currency), [...slot.adjustments, pricingRule.adjustment])
    }
    return pricedSlot(slot.slot, price(slot.price.amount.value + (slot.price.amount.value * pricingRule.adjustment.percentage), slot.price.currency), [...slot.adjustments, pricingRule.adjustment])
}

function applyTimeBasedPriceAdjustment(slot: PricedSlot, pricingRule: TimeBasedPriceAdjustment): PricedSlot {
    if (dayAndTimePeriodFns.overlaps(pricingRule.time, dayAndTimePeriod(slot.slot.date, calcSlotPeriod(slot.slot.slot, slot.slot.service.duration)))) {
