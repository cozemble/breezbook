...
export function logRequest(req: Request) {
	if (!req.path.startsWith('/internal') || (!req.path.startsWith('/api') && req.path.includes('inngest'))) {
		return;
	}
	...
	const lines = ['Content-Type', 'Referer', 'Content-Length', 'X-Forwarded-For', 'User-Agent'].reduce(
		(acc, header) => {
